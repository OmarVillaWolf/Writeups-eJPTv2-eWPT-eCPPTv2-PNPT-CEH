...
❯ poetry run cme smb ❮IP❯ -u 'User' -p 'Pass'      # Verificar si el usuario y su password es valida. Si el usuario es 'Administrator' el comando mostrará un [Pwn3d!] y se podra conectar con 'Psexec'

❯ poetry run cme smb ❮IP❯ -u users.txt -p Pass.txt --no-brute --continue-on-success  # Ataque de diccionario probando el usuario y la password de forma paralela 
❯ poetry run cme smb ❮IP❯ -u users.txt -p Pass.txt --continue-on-success     # Ataque de diccionario

❯ poetry run cme winrm ❮IP❯ -u 'User' -p 'Pass'    # Validar si el usuario y la passwd son validas. Si el usuario pertenece al grupo 'Remote Management Users' el comando mostrará un [Pwn3d!] y se podra conectar con 'Evil-winrm' por el puerto 5985
```
